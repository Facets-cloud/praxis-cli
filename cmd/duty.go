...
func resolveAgentID(out io.Writer, active credentials.Active, agentArg string) string {
	agents, err := agentcatalog.FetchIncludingGlobal(active.Profile.URL, active.Profile.AuthHeader())
	if err != nil {
...
func resolveScheduleID(out io.Writer, active credentials.Active, agentID, dutyArg string) string {
	schedules, err := duties.ListSchedules(active.Profile.URL, active.Profile.AuthHeader(), agentID, "")
	if err != nil {
		return reportResolveErr(out, active.Name, err)
	}
...
var dutyRunsCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		runs, err := duties.ListRuns(active.Profile.URL, active.Profile.AuthHeader(), agentID, scheduleID, dutyRunsLimit)
		if err != nil {
			return reportHTTPErr(out, active.Name, err)
		}
		if runs == nil {
...
var dutyRunCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		run, err := duties.GetRun(active.Profile.URL, active.Profile.AuthHeader(), agentID, args[0])
		if err != nil {
			return reportHTTPErr(out, active.Name, err)
		}
		if asJSON {
...
var dutyReportCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		body, mime, err := duties.FetchArtifactContent(active.Profile.URL, active.Profile.AuthHeader(), *run.ReportArtifactID)
		if err != nil {
			return reportHTTPErr(out, active.Name, err)
		}
		if asJSON {
...
var dutyFindingsCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		findings, err := duties.ListFindings(active.Profile.URL, active.Profile.AuthHeader(), agentID, scheduleID, dutyFindingsStatus, dutyFindingsLimit)
		if err != nil {
			return reportHTTPErr(out, active.Name, err)
		}
		if findings == nil {
